...
_main() {
    ...
    if [ "$USE_BASEBACKUP" = "1" ]; then
    	if [ -d "$PGDATA" ] && [ "$(ls -A $PGDATA)" ]; then
    	echo "Data directory '$PGDATA' is not empty."
			else    		
      echo "Running pg_basebackup before starting PostgreSQL..."
      PGPASSWORD="${PG_MASTER_PASSWORD}" pg_basebackup -D "$PGDATA" -h "${PG_MASTER_HOST}" -p "${PG_MASTER_PORT}" -U "${PG_MASTER_USER}" -X stream -P
			fi
